...
export default function ChannelPartnersPage() {
  ...
  const fetchCPLeads = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/leads?category=CHANNEL_PARTNER&page=${page}&pageSize=${pageSize}`);
      if (res.ok) {
      ...
      <div className="flex items-center justify-between pb-4 border-b border-[#E5E5E0]">
        <div>
          <h2 className="text-xl font-bold text-[#171817] flex items-center gap-2">
            <Share2 className="w-5 h-5 text-[#B69A63]" /> Channel Partner Network
          </h2>
          <p className="text-xs text-[#626560] mt-1">
            Dedicated portal for channel partners, broker syndicates, and institutional associates.
          </p>
        </div>

        <Button size="sm" variant="secondary" onClick={fetchCPLeads} isLoading={loading}>
